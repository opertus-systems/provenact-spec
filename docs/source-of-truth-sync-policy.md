...
This policy prevents silent drift for artifacts duplicated across Provenact repositories.
...
- Source repo: `provenact-spec`
- Source paths:
  - `spec/`
  - `test-vectors/`
- Mirrors:
  - `provenact-cli/spec/`
  - `provenact-cli/test-vectors/`

### 2) Control plane OpenAPI contract
- Source repo: `provenact-control`
- Source path:
  - `openapi.yaml`
- Mirror:
  - `provenact-control-web/public/openapi.yaml`
...
### Spec/vector sync (`provenact-spec` -> `provenact-cli`)

1. Change lands in `provenact-spec`.
2. `provenact-spec` tags or publishes source commit.
3. Sync PR in `provenact-cli` copies `spec/` + `test-vectors/` verbatim.
4. Sync PR includes machine-generated manifest:
   ...
   - file count
   - checksum summary
   - command: `tools/generate-sync-manifest.sh spec-cli <provenact-cli-path>`
5. CI in `provenact-cli` validates:
   - parity with declared source commit
   - conformance tests still pass

### OpenAPI sync (`provenact-control` -> `provenact-control-web`)

1. API contract change merges in `provenact-control`.
2. Sync PR copies `openapi.yaml` to web mirror path.
   - command: `tools/generate-sync-manifest.sh openapi <provenact-control-web-path>`
3. CI in `provenact-control-web` verifies checksum parity.
4. Docs/examples regenerated in same PR if needed.

## Required CI Gates

### In `provenact-cli`
1. `sync-spec-check`: compare mirrored `spec/` + `test-vectors/` with pinned `provenact-spec` commit.
...
### In `provenact-control-web`
1. `sync-openapi-check`: compare `public/openapi.yaml` with pinned `provenact-control` commit.
...
1. `provenact-spec` maintainers own normative contract acceptance.
2. `provenact-control` maintainers own API contract acceptance.
